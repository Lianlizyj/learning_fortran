...
x2 = x2(:);
y2 = y2(:);
% add this code to avoid error caused by sine function.(i.e. in
% princeple,sin(k*pi)==0, while k is an integer. 
% After tested on Maltab,I found 'abs(sin(k*pi))<=eps' can't be satisfied except k=0,1,29,2*29,3*39,...,128*29.)
% Since 'pi' is an irrational, there can exists some error in the computation of 'sin(n*pi)' which was computated by series.
% It's so sorry that we don't have any good method to jadge if a value is the result of
% computation 'sin(n*pi)', where n satisfys 'abs(n)~=k'(as put above).
idx=abs(y1-y2)<=eps; y2(idx)=y1(idx);
